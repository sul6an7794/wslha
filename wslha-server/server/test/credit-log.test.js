...
test('إنشاء حساب جديد يسجّل تذكرة البداية بسجل التدقيق', async () => {
  const user = await db.insertUser({ username: 'clog_signup', password_hash: 'x' });
...
test('addCredits يسجّل الخصم/الإضافة مع السبب والرصيد بعده', async () => {
  const user = await db.insertUser({ username: 'clog_add', password_hash: 'x' });
...
test('setUserCredits (ضبط المشرف اليدوي) يسجّل الفرق فقط لا القيمة المطلقة', async () => {
  const user = await db.insertUser({ username: 'clog_admin', password_hash: 'x' });
...
test('setUserCredits بدون تغيير فعلي ما يضيف سجل جديد', async () => {
  const user = await db.insertUser({ username: 'clog_nochange', password_hash: 'x' });
...
test('getCreditLog يعزل سجل كل مستخدم عن غيره ويرجع الأحدث أولًا', async () => {
  const a = await db.insertUser({ username: 'clog_a', password_hash: 'x' });
  const b = await db.insertUser({ username: 'clog_b', password_hash: 'x' });
